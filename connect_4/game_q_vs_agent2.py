...
def main():
    ...
    """Q-learning vs DQN"""
    # Displaying and playing the first 10 games with GUI for Q-Learning vs. DQN
    print("Displaying the first 10 games (Q-Learning vs. DQN):")
    for i in range(10):
        winner = simulate_game(q_agent, dqn_agent, display=True)
        winner_text = "Draw" if winner is None else "Q-Learning Agent" if winner == 'X' else "DQN Agent"
    ...
    for _ in range(num_games):
        winner = simulate_game(q_agent, dqn_agent)
        if winner == 'X':
            q_wins += 1
        elif winner == 'O':
            dqn_wins += 1
    ...
    draws = num_games - q_wins - dqn_wins
    ...
    labels = ['Q-Learning Wins', 'DQN Wins', 'Draws']
    counts = [q_wins, dqn_wins, draws]

    plt.figure(figsize=(10, 6))
    bars = plt.bar(labels, counts, color=['blue', 'red', 'gray'])
    plt.xlabel('Results')
    plt.ylabel('Counts')
    plt.title(f'Q-Learning vs. DQN in {num_games} Connect Four Games')
    for bar in bars:
        yval = bar.get_height()
    ...
    """Q-learning vs SARSA"""
    # Displaying and playing the first 10 games with GUI for Q-learning vs SARSA
    print("Displaying the first 10 games (Q-learning vs SARSA):")
    for i in range(10):
        winner = simulate_game(q_agent, sarsa_agent, display=True)
        winner_text = "Draw" if winner is None else "Q-learning Agent" if winner == 'X' else "SARSA Agent"
        print(f"Game {i+1} Winner: {winner_text}")
    ...
    num_games = Config.NUM_EPISODES - 10
    for _ in range(num_games):
        winner = simulate_game(q_agent, sarsa_agent)
        if winner == 'X':
            q_wins += 1
        elif winner == 'O':
            sarsa_wins += 1
    ...
    # Displaying the results
    draws = num_games - q_wins - sarsa_wins
    print(f"\nResults of {num_games} games:")
    print(f"Q-learning Agent wins: {q_wins}")
    ...
    # Plotting the results
    labels = ['Q-learning Wins', 'SARSA Wins', 'Draws']
    counts = [q_wins, sarsa_wins, draws]

    plt.figure(figsize=(10, 6))
    bars = plt.bar(labels, counts, color=['blue', 'green', 'gray'])
    plt.xlabel('Results')
    plt.ylabel('Counts')
    plt.title(f'Q-learning vs. SARSA in {num_games} Connect Four Games')
